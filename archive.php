...
$stmt = $conn->prepare("SELECT * FROM tasks WHERE UserID = ? AND Status = 'Archive' ORDER BY Deadline ASC");
$stmt->bind_param('i', $user_id);
$stmt->execute();
$result = $stmt->get_result();
$archived_tasks = $result->fetch_all(MYSQLI_ASSOC);
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_task'])) {
    ...
    // Check if any tasks are selected for deletion
    if (!empty($task_ids)) {
        $ids = implode(',', array_map('intval', $task_ids)); // Sanitize task IDs for SQL
        $delete_stmt = $conn->prepare("DELETE FROM tasks WHERE TaskID IN ($ids)");
        ...
        if ($delete_stmt->execute()) {
            $_SESSION['message'] = "Selected tasks deleted successfully."; // Optional success message
        } else {
            $_SESSION['error'] = "Failed to delete tasks: " . $delete_stmt->error;
            error_log("Error deleting task IDs $ids: " . $delete_stmt->error); // Log the error
        }

        $delete_stmt->close();
    } else {
        $_SESSION['error'] = "No tasks selected for deletion.";
    }
...
<div class="container mt-5 p-4" style="background-color: white; border-radius: 20px; box-shadow: 0px 4px 15px rgba(0, 0, 0, 0.1);">
    <h1 class="text-center mb-4" style="font-weight: bold; color: #7d7d7d;">Archived Tasks</h1>
    ...
    <?php if (count($archived_tasks) > 0): ?>
        <form method="post" action="">
                ...
                <table class="table table-hover table-bordered" style="border-radius: 15px; box-shadow: 0px 4px 12px rgba(0, 0, 0, 0.1);">
                    ...
                    <tbody>
                        <?php foreach ($archived_tasks as $task): ?>
                            <tr class="text-center" style="color: black; background: linear-gradient(135deg, #7d7d7d, #b0b0b0);">
                                <td>
            ...
            <button type="submit" name="delete_task" class="btn btn-danger" style="border-radius: 20px; padding: 8px 16px; margin-left: 10px;">
                <i class="fa fa-trash"></i> Delete Selected
            </button>
        </form>
    <?php else: ?>
        <p class="text-center text-muted" style="font-size: 18px;">No archived tasks found.</p>
    <?php endif; ?>
</div>
