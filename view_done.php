...
// Fetch tasks with Done status that are within the current month
$stmt = $conn->prepare("SELECT * FROM tasks WHERE UserID = ? AND Status = 'Done' AND Deadline BETWEEN ? AND ? ORDER BY Deadline ASC");
$stmt->bind_param('iss', $user_id, $first_day_of_month, $last_day_of_month);
$stmt->execute();
$result = $stmt->get_result();
$done_tasks = $result->fetch_all(MYSQLI_ASSOC);
$stmt->close();
...
<div class="container mt-5 p-4" style="background-color: white; border-radius: 20px; box-shadow: 0px 4px 15px rgba(0, 0, 0, 0.1);">
    <h1 class="text-center mb-4" style="font-weight: bold; color: #488c44;">Done Tasks</h1>
                ...
                <table class="table table-hover table-bordered" style="border-radius: 15px; box-shadow: 0px 4px 12px rgba(0, 0, 0, 0.1);">
                    ...
                    <tbody>
                        <?php foreach ($done_tasks as $task): ?>
                            <tr class="text-center" style="color: white; background: linear-gradient(135deg, #488c44, #68b764);">
                                <td>
                                ...
                                <td><?php echo htmlspecialchars(date('F j, Y', strtotime($task['Deadline']))); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
    ...
    <?php else: ?>
        <p class="text-center text-muted" style="font-size: 18px;">No done tasks found.</p>
    <?php endif; ?>
</div>
