...
// Fetch tasks with Priority status that are within the current month
$stmt = $conn->prepare("SELECT * FROM tasks WHERE UserID = ? AND Status = 'Priority' AND Deadline BETWEEN ? AND ? ORDER BY Deadline ASC");
$stmt->bind_param('iss', $user_id, $first_day_of_month, $last_day_of_month);
$stmt->execute();
$result = $stmt->get_result();
$priority_tasks = $result->fetch_all(MYSQLI_ASSOC);
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if (!empty($task_ids)) {
        ...
        if ($update_stmt === false) {
            ...
            header("Location: view_priority.php");
            exit;
        }
...
<div class="container mt-5 p-4" style="background-color: white; border-radius: 20px; box-shadow: 0px 4px 15px rgba(0, 0, 0, 0.1);">
   <h1 class="text-center mb-4" style="font-weight: bold; color: #e14b4b;">Priority Tasks</h1>
            ...
            <div class="task-container">
                <table class="table table-hover table-bordered" style="border-radius: 15px; box-shadow: 0px 4px 12px rgba(0, 0, 0, 0.1);">
                    ...
                    <tbody>
                        ...
                        $task_count = min(count($priority_tasks), 10);
                        for ($i = 0; $i < $task_count; $i++): 
                            $task = $priority_tasks[$i]; ?>
                            <tr class="text-center" style="color: white; background: linear-gradient(135deg, #e14b4b, #f75d5d);">
                                <td>
    ...
    <?php else: ?>
        <p class="text-center text-muted" style="font-size: 18px;">No priority tasks found.</p>
    <?php endif; ?>
</div>
<br>
<!-- Footer -->
