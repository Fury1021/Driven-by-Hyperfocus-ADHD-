...
// Fetch tasks with Side Task status that are within the current month
$stmt = $conn->prepare("SELECT * FROM tasks WHERE UserID = ? AND Status = 'Side Task' AND Deadline BETWEEN ? AND ? ORDER BY Deadline ASC");
$stmt->bind_param('iss', $user_id, $first_day_of_month, $last_day_of_month);
$stmt->execute();
$result = $stmt->get_result();
$side_tasks = $result->fetch_all(MYSQLI_ASSOC);
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if (!empty($task_ids)) {
        ...
        if ($update_stmt === false) {
            ...
            header("Location: view_side_task.php");
            exit;
        }
...
<div class="container mt-5 p-4" style="background-color: white; border-radius: 20px; box-shadow: 0px 4px 15px rgba(0, 0, 0, 0.1);">
    <h1 class="text-center mb-4" style="font-weight: bold; color: #2064cc;">Side Tasks</h1>
        ...
        <form method="post" action="">
            <div class="table-responsive">
                <table class="table table-hover table-bordered" style="border-radius: 15px; box-shadow: 0px 4px 12px rgba(0, 0, 0, 0.1);">
                    ...
                    <tbody>
                        <?php foreach ($side_tasks as $task): ?>
                            <tr class="text-center" style="color: white; background: linear-gradient(135deg, #2064cc, #2064cc);">
                                <td>
    ...
    <?php else: ?>
        <p class="text-center" style="font-weight: 600; color: #2064cc;">No side tasks found.</p>
    <?php endif; ?>
</div>
 <!-- Footer -->
